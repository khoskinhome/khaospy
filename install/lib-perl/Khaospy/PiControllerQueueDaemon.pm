...
use Khaospy::Log qw/ log START FATAL ERROR WARN INFO DEBUG /;
...
sub run_controller_queue_daemon {
    ...
    log(START,"Controller Queue Daemon START");
    log(START,"VERBOSE = ".( $VERBOSE ? "TRUE" : "FALSE" ));

    $zmq_publisher  = zmq_socket($ZMQ_CONTEXT, ZMQ_PUB);
    my $pub_to_port = "tcp://*:$PI_CONTROLLER_QUEUE_DAEMON_SEND_PORT";
    zmq_bind( $zmq_publisher, $pub_to_port );
    log(INFO, "Publishing to $pub_to_port");
    ...
    log(INFO, "Listening (REP) to $connect_str");

    if ( my $zmq_state = zmq_connect($zmq_reply_sock, $connect_str )){
        log(FATAL, "zmq can't connect to $connect_str. status = $zmq_state. $!");
    };
...
sub timer_cb {
    ...
    for my $mkey ( keys %$msg_queue ){
        ...
        log(INFO, "Publish message $msg_queue->{$mkey}{json}");
        zhelpers::s_send( $zmq_publisher, $msg_queue->{$mkey}{json} );

        if ( $msg_rh->{epoch_time} < time - $MESSAGE_TIMEOUT ){
            log(ERROR, "Msg timed out. $mkey");
...
sub queue_message {
    ...
    if ($@){
        ...
        zhelpers::s_send( $zmq_sock, "ERROR. Not queued" );
        return;
    }
    ...
    log(DEBUG, "Reply to requestor $msg");
    zhelpers::s_send( $zmq_sock, "queued $msg_p->{mkey}" );

    # publish this message for control-daemons to grab :
    log(DEBUG, "Publish (first) message $msg");
    zhelpers::s_send( $zmq_publisher, $msg );
...
sub message_acknowleged_by_controller {
    ...
    log(INFO, "Control Daemon Acknowledges message $msg");
    #my ($topic, $msgdata) = $msg =~ m/(.*?)\s+(.*)$/;
