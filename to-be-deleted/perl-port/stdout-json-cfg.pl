...
my $conf = {
    ...
    testautomationlight => { # using a lighting module to do the "manual" switch when automatically testing
        HomeAutoClass       => "khaospy.mcp23017MultiLightSingleWayManual.Device",
        Hostname            => "piold", # 192.168.1.10,
        Ports => {
            ChangeOver => { # relay-1 on pcb # changeover # white-signal-wire # formerly auto_light_1
                enabled         => 1,
                i2cbus          => $i2cbus->{testautomationlight},
                i2cAddress      => '0x27', # i2c address of the mcp23017
                port            => 'b',    # a or b ONLY
                portnum         => 4,      # 0 -> 7
                initial         => 1,
            ...
            LightSwitchMains => { # formerly auto_light_switch_detect # detect 240v on light switch
                enabled         => 1,
                i2cbus          => $i2cbus->{testautomationlight},
                i2cAddress      => '0x27',
                port            =>'b',     # a or b ONLY
            ...
            LightSwitchExtra => { # spare 5 Volt input switch. formerly auto_light_extra_switch_detect
                enabled         => 0,
                i2cbus          => $i2cbus->{testautomationlight},
                i2cAddress      => '0x27',
                port            =>'b',     # a or b ONLY
            ...
            Lights => [
                {  # relay-2 on pcb # light-auto-on-off # blue-signal-wire # formerly auto_light_2
                    enabled         => 0,
                    i2cbus          => $i2cbus->{testautomationlight},
                    i2cAddress      => '0x27', # i2c address of the mcp23017
                    port            => 'b',     # a or b ONLY
