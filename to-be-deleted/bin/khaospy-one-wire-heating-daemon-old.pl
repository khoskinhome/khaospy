...
# being renamed from KHAOSPY_ONE_WIRE_HEATING_DAEMON to KHAOSPY_ONE_WIRE_HEATING_DAEMON
...
;
    ...
    if ( ! $control_name || ! $upper_temp || ! $lower_temp ){
        ...
        print "    Please fix the config file $HEATING_DAEMON_CONF_FULLPATH\n";
    ...
    if ( $upper_temp <= $lower_temp ){
        print "\n    Broken temperature range in $HEATING_DAEMON_CONF_FULLPATH config.\n";
